...
#    model.OBJ = pyo.Objective(rule = ObjRule, sense = pyo.minimize)  
    ...
    """
    def min_gen_2(model): #Minimum power generated by gen 2
        clean_power = sum(model.gen[g] for g in model.generators if g in Clean_generators)
        total_power = sum(model.gen[g] for g in model.generators)
        return(clean_power >= 0.2 * total_power)
    model.minimum_gen_2 = pyo.Constraint(rule = min_gen_2)
    """
    
    def cap_and_trade(model): #Rule for cap and trade
